...
export default function UserDashboard() {
  ...
  // Fetch unread notifications with a low limit, but use meta.total for real count
  const { data: notifications, isLoading: notifLoading } = useQuery({
    queryKey: ['userNotifications', 'unread', 'count'],
    queryFn: () => getNotifications({ unreadOnly: true, limit: 1 }), // limit 1, we only need meta.total
  });

  if (sessLoading || notifLoading) return <Loader fullScreen />;

  const activeSessions = sessions?.data?.data?.length || 0;
  const unreadCount = notifications?.data?.meta?.total || 0;
